...
public class SignChangeListener extends EventListener {
  ...
  private void handleSignNearbyProtection(SignChangeEvent event, Protection protection) {
    ...
    if (newSignType.filter(SignType::isMainSign).isPresent()) {
      ...
      if (oldSignType.isPresent()) {
        ...
        plugin
            .getTranslator()
            .sendMessage(player, Translation.PROTECTION_ADD_MORE_USERS_SIGN_INSTEAD);
  ...
  private void handleSignNotNearbyProtection(SignChangeEvent event) {
    ...
    // Check event
    if (this.plugin
        .callEvent(new PlayerProtectionCreateEvent(event.getPlayer(), block))
        .isCancelled()) {
      block.breakNaturally();
      event.setCancelled(true);
      return;
  ...
  public void onSignChange(SignChangeEvent event) {
    Optional<Protection> protection =
        plugin.getProtectionFinder().findExistingProtectionForNewSign(event.getBlock());
  ...
  /**
   * Call this method for newly created protections. The next tick the protection will be found and
   * updated for uuids.
   *
   * @param block The block that will be part of the protection.
   */
